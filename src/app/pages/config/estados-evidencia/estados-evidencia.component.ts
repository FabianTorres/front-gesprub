// src/app/pages/config/estados-evidencia/estados-evidencia.component.ts
...
import { EstadoEvidenciaService } from '../../../services/estado-evidencia.service';

@Component({
  selector: 'app-estados-evidencia',
  standalone: true,
  ...
  templateUrl: './estados-evidencia.component.html'
})
export class EstadosEvidenciaComponent implements OnInit {

  private estadoService = inject(EstadoEvidenciaService);
  private messageService = inject(MessageService);

  estados = signal<EstadoEvidencia[]>([]);
  estadoDialog: boolean = false;
  estado!: Partial<EstadoEvidencia>;
  ...
  abrirDialogoEditar(estadoAEditar: EstadoEvidencia) {
  ...
  guardarEstado() {
    ...
    const peticion = this.editando
      ? this.estadoService.updateEstado(this.estado.id_estado_evidencia!, this.estado)
      ...
      next: (estadoGuardado) => {
        ...
        if (this.editando) {
          this.estados.update(lista => {
            const index = lista.findIndex(e => e.id_estado_evidencia === estadoGuardado.id_estado_evidencia);
